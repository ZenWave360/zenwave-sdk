...
- Kafka consumers generated by `asyncapi-generator` can now be disabled per environment with `app.kafka.topics.<channel>.enabled=false`; consumers remain enabled by default and the topic property remains required.
